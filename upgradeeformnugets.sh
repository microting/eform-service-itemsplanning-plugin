...
if (( "$GIT_STATUS" > 0 )); then
  ...
  CURRENT_NUMBER_OF_COMMITS=`git log --oneline | wc -l`
	...
	PACKAGES=('Microting.eForm' 'Microting.eFormApi.BasePn' 'Microting.ItemsGroupPlanningBase')
	PROJECT_NAME='ServiceItemsGroupPlanningPlugin.csproj'
	REPOSITORY='eform-service-items-group-planning-plugin'
